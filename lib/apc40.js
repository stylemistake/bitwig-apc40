...
function APC40( midi_in, midi_out ) {
	...
	function onMidi( status, data1, data2 ) {
		...
		if ( isControl( status ) && data1 === 0x07 ) {
			return events.fire( "track_volume_change", status & 0x0, data2 );
		}
	...
	this.setActivatorLed = function( id, value ) {
		return dispatcher.send( ( value === 0 ? 0x80 : 0x90 ) | id, 0x32, value );
	}

	this.setSoloLed = function( id, value ) {
		return dispatcher.send( ( value === 0 ? 0x80 : 0x90 ) | id, 0x31, value );
	}

	this.setArmLed = function( id, value ) {
		return dispatcher.send( ( value === 0 ? 0x80 : 0x90 ) | id, 0x30, value );
	}
